#is a replacement for the change of GRBL to custom codefrom queue import Empty
from sys import platform
import time
import serial
import serial.tools.list_ports as port_list

class serial_bridge_CUSTOM:
    ...
    def bridge(self):
        ...
        if platform == "linux" or platform == "linux2":
            ...
            ports = self.check_ports()
            s = self.connect_to_ports_win(ports, "HangingArm")
            return s
    ...
    def send_move(self, input):
        if self.device is None:
           ...
           return
        print(input)
        self.device.write(bytearray("M "+str(input)+"\r\n","utf-8"))
